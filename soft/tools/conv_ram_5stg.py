...
def mem_to_vhdl_slv32(input_file, output_file):
    ...
    vhdl_lines.append(f"")
    vhdl_lines.append(f"entity mem_ram_dp is ")
    vhdl_lines.append(f"\tPort (")
    vhdl_lines.append(f"\t\tCLOCK    : IN  STD_LOGIC;")
    vhdl_lines.append(f"\t\tADDR_R   : IN  STD_LOGIC_VECTOR(RAM_ADDR-1  DOWNTO 0);")
    vhdl_lines.append(f"\t\tDATA_R   : OUT STD_LOGIC_VECTOR(RAM_WIDTH-1 DOWNTO 0);")
    vhdl_lines.append(f"\t\tADDR_W   : IN  STD_LOGIC_VECTOR(RAM_ADDR-1  DOWNTO 0);")
    ...
    vhdl_lines.append(f"\t);")
    vhdl_lines.append(f"end mem_ram_dp;")
    vhdl_lines.append(f"")
    vhdl_lines.append(f"")
    vhdl_lines.append(f"architecture arch of mem_ram_dp is")
    vhdl_lines.append(f"")
    ...
    vhdl_lines.append(f"\tSIGNAL W_ADDR : STD_LOGIC_VECTOR(RAM_ADDR-2-1 DOWNTO 0); -- -2 bits because 32b words and not bytes")
    vhdl_lines.append(f"")
    vhdl_lines.append(f"begin")
    vhdl_lines.append(f"")
    vhdl_lines.append(f"\tR_ADDR <= ADDR_R(RAM_ADDR-1 DOWNTO 2);")
    vhdl_lines.append(f"\tW_ADDR <= ADDR_W(RAM_ADDR-1 DOWNTO 2);")
    vhdl_lines.append(f"")
    ...
    vhdl_lines.append(f"\t\t\taddr := TO_INTEGER( unsigned(W_ADDR) );")
