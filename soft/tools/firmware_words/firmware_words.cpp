...
int load_RAM_elf(const char* filename, std::vector<unsigned char>& RAM) {
    ...
    if(info.max_address >= RAM_SIZE) {
		std::cerr << "(WW) Memory limit is exceeded !" << std::endl;
		std::cerr << "(WW) If your a not in a sliced memory configuration it is a probleme" << std::endl;
		std::cerr << "(WW) Error => Warning @ bertrand.le-gal@univ-rennes.fr" << std::endl;
//		return -1;
    }
...
//	std::cout << "testing MAX_ADDR limit: " << MAX_ADDR << std::endl;
	if(max_addr > MAX_ADDR) {
	    std::cerr << "(WW) max_addr overflow, MAX_ADDR exceeded" << std::endl;
		std::cerr << "(WW) Error => Warning @ bertrand.le-gal@univ-rennes.fr" << std::endl;
//	    return 1;
	} else {
