...
end riscv_soc_altera;

architecture arch of riscv_soc_altera is
...
--    SIGNAL data_from_spi_busy  : STD_LOGIC;
	...
	SIGNAL   leds_buffer        : STD_LOGIC_VECTOR ( 7 downto 0);
...
-- data_to_ledrgb_wen   <= data_to_ledrgb_en   and dm_wr;
   ...
   BEGIN
      if rising_edge(CLOCK) then
         RESET  <=  NOT BTN(0); -- on tang nano board the button are inverted
         RESETN <=      BTN(0); -- meaning, equals '1' when unpressed
         BUTTON <= NOT  BTN(1);
      END IF;
   END PROCESS;
   ...
   begin   
      if rising_edge(CLOCK) then
         IF RESET = '1' THEN
            leds_buffer <= "00000000";
         ELSIF data_to_leds_wen = '1' then
            leds_buffer <= dm_wdata(7 downto 0);
         end if;
      end if;
   end process;

   LED(7 DOWNTO 0) <= leds_buffer when RESET = '0' else "1110111" & BUTTON;
   ...
   begin   
      if rising_edge(CLOCK) then
         d32b_from_swit <= x"00000"   & "00"  & SW;
         d32b_from_btns <= x"0000000" & "000" & BUTTON;
      end if;
   end process;
   ...
   d32b_from_leds   <= x"000000" &  leds_buffer;
...
--          else d32b_from_led_rgb   when data_to_ledrgb_en   = '1'
   ...
   d32b_from_soc_id <= x"44453130"                                   WHEN dm_addr(4 downto 2) = "000" else -- DE-10 Lite SoC
                       STD_LOGIC_VECTOR( TO_UNSIGNED(50000000, 32) ) WHEN dm_addr(4 downto 2) = "001" else -- frequence core
                       STD_LOGIC_VECTOR( TO_UNSIGNED(      10, 32) ) WHEN dm_addr(4 downto 2) = "010" else -- #leds
                       STD_LOGIC_VECTOR( TO_UNSIGNED(       1, 32) ) WHEN dm_addr(4 downto 2) = "011" else -- #boutons
                       STD_LOGIC_VECTOR( TO_UNSIGNED(      10, 32) ) WHEN dm_addr(4 downto 2) = "100" else -- #switches
